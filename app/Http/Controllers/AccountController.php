...
class AccountController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($accounts)
                ...
                ->editColumn('parent_account_id', function ($row) use($parentAccounts) {
                    if ($row->parent_account_id && array_key_exists($row->parent_account_id,$parentAccounts)) {
        ...
        return view('account.index')
            ->with(compact('parentAccounts','filterdata','account_types_opts','sub_acn_arr','not_linked_payments', 'account_types', 'account_access', 'business_locations', 'account_groups', 'users', 'accounts','suppliers','orderStatuses'));
    }
    /**
    ...
    public function create()
    {
        ...
        return view('account.create')
            ->with(compact('account_types', 'account_groups', 'asset_type_ids', 'accounts','parentAccounts'));
    }
    /**
    ...
    public function edit($id)
    {
        ...
        if (request()->ajax()) {
            ...
            return view('account.edit')
                ->with(compact('account', 'account_types', 'balance', 'account_groups', 'account_access','selected_account_group', 'asset_type_ids', 'start_date', 'parent_accounts'));
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (request()->ajax()) {
            try {
                $user_id = $request->session()->get('user.id');
                $input = $request->only(['name', 'account_number','is_need_cheque', 'note', 'account_type_id', 'parent_account_id', 'is_main_account', 'asset_type', 'increase_reduce', 'transaction_date']);
                $business_id = request()->session()->get('user.business_id');
                ...
                if ($account->save()) {
                    ...
                    foreach($optrans as $tras){
                        $account_type_name = AccountType::where('id', $tras->account_type_id)->first();
                        $type = 'debit';
                        if (strpos($account_type_name, "Assets") !== false || strpos($account_type_name, "Expenses") !== false) {
                            if ($tras->amount >= 0) {
                                $type = 'debit';
